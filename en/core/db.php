...
class db extends mysqli{
	...
	public function getMaterial($name, $fields=["content"]){
		$row = $this->getRow("
		SELECT ".implode(",", $fields)." FROM gb_sitemap USE INDEX (sounding)
		CROSS JOIN gb_pages USING(PageID) CROSS JOIN gb_static USING(PageID)
		WHERE soundex LIKE {str} AND name LIKE {str} ORDER BY (POW(2,language-1) & {int}) LIMIT 1", soundex($name),$name,LANG_MASK);
		return $row;
	}
	public function getPost($id){
		...
		CROSS JOIN gb_staff USING(UserID) CROSS JOIN gb_community USING(CommunityID)
		WHERE ID = {int} ORDER BY (POW(2,language-1) & {int}) LIMIT 1", $id, LANG_MASK);
		return $row;
	}
