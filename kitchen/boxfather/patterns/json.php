<?php
function patterns_tree($root="patterns/json", $level=0){
...
?>
<div class="box-caption logo-bg">JSON<?include_once("components/movebox.php")?></div>
<div class="h-bar logo-bg">
	<input name="path" value="<?=((PATH)?$path['dirname']:'patterns/json')?>" required>
	<input name="pname" placeholder="pattern name" value="<?=((PATH)?$path['filename']:'')?>" data-translate="placeholder" pattern="[a-zA-Z0-9_-]+" required>
	<div class="toolbar">
...
<div class="box-body">
	...
	<?if(PATH):?>
	<main><?=file_get_contents(PATH)?></main>
	<?else:?>
	<main></main>
	<?endif?>
</div>
