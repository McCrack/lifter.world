	...
	case "status":
		...
		if($changed>0){
			if($p['status']=="new"){
				...
				print $mySQL->inquiry("UPDATE gb_task_timing SET towork={int}, log=CONCAT(log,{str}) WHERE TaskID={int} LIMIT 1", time(),$log,ARG_3);
			}else print $mySQL->inquiry("UPDATE gb_task_timing SET towaste={int}, log=CONCAT(log,{str}) WHERE TaskID={int} LIMIT 1", time(),$log,ARG_3);
		}
